...
use crate::crypto::{generate_salt, hash_password, verify_password, derive_key, encrypt, decrypt, encrypt_gcm, decrypt_gcm, CryptoError};
use chrono::Utc;
use serde_json;
...
fn rename_with_retry(from: &std::path::Path, to: &std::path::Path) -> std::io::Result<()> {
    ...
    Err(last_err.unwrap_or_else(|| std::io::Error::new(std::io::ErrorKind::Other, "rename 重试失败")))
}
...
pub fn set_master_password(store: &mut StoreData, password: &str) -> Result<(), StorageError> {
    ...
    store.master_password_hash = hash_password(password, &store.salt, 1)?;
    store.key_derivation_version = 1;
...
/// 迁移密钥派生算法到 V2 + GCM 加密（在旧版验证成功后调用）
///
...
pub fn migrate_key_derivation(
    ...
    if store.key_derivation_version >= 1 && store.encryption_version >= 1 {
        return Ok(());
    }

    let old_key = derive_key(password, &store.salt, store.key_derivation_version)?;
    let new_key = derive_key(password, &store.salt, 1)?;
...
pub fn change_master_password(
    ...
    let new_key = derive_key(new_password, &store.salt, 1)?;
    ...
    store.master_password_hash = hash_password(new_password, &store.salt, 1)?;
    store.key_derivation_version = 1;
    store.encryption_version = 1;
    store.updated_at = Utc::now();
    save_store(store)?;
...
/// 从分组中移除凭据 ID
pub fn remove_credential_from_group(
    store: &mut StoreData,
    credential_id: &str,
) {
...
/// 新建用户自定义分组
pub fn add_group(store: &mut StoreData, name: &str) -> Group {
    let group = Group {
...
/// 重命名分组（系统/默认分组不可重命名）
pub fn rename_group(store: &mut StoreData, group_id: &str, new_name: &str) -> Result<(), StorageError> {
    if is_group_protected(store, group_id) {
        return Err(StorageError::SystemGroupProtected);
    }
    let group = store.groups.iter_mut()
        .find(|g| g.id == group_id)
        .ok_or(StorageError::GroupNotFound)?;
    group.group_name = new_name.to_string();
    group.updated_at = Utc::now();
    Ok(())
}

/// 删除分组：条目移入默认分组
pub fn delete_group(store: &mut StoreData, group_id: &str) -> Result<(), StorageError> {
    if is_group_protected(store, group_id) {
        return Err(StorageError::SystemGroupProtected);
    }
    let default_group_id = get_default_group_id(store);
    if let Some(ref default_gid) = default_group_id {
        for cred in &mut store.credentials {
            if cred.group_id.as_deref() == Some(group_id) {
                cred.group_id = Some(default_gid.clone());
            }
        }
        // Collect entry_ids first to avoid borrow conflict
        let entry_ids: Vec<String> = store.groups.iter()
            .find(|g| g.id == group_id)
            .map(|g| g.entries.clone())
            .unwrap_or_default();
        if let Some(default_group) = store.groups.iter_mut().find(|g| g.id == *default_gid) {
            for eid in entry_ids {
                if !default_group.entries.contains(&eid) {
                    default_group.entries.push(eid);
                }
            }
...
/// 删除分组并选择条目处理方式：move=移入默认组, delete=连同凭据一起删除
pub fn delete_group_with_options(store: &mut StoreData, group_id: &str, mode: &str) -> Result<usize, StorageError> {
    if is_group_protected(store, group_id) {
        return Err(StorageError::SystemGroupProtected);
    }
    let entry_ids: Vec<String> = store.groups.iter()
    ...
    if mode == "delete" {
        store.credentials.retain(|c| !entry_ids.contains(&c.id));
    } else {
        let default_group_id = get_default_group_id(store);
        if let Some(ref default_gid) = default_group_id {
            for cred in &mut store.credentials {
...
/// 批量移动凭据到指定分组
pub fn batch_move_to_group(store: &mut StoreData, cred_ids: &[String], group_id: &str) -> Result<usize, StorageError> {
    let mut count = 0;
    for id in cred_ids {
        for group in &mut store.groups {
            group.entries.retain(|eid| eid != id);
        }
        let _ = add_credential_to_group(store, id, group_id);
        if let Some(cred) = store.credentials.iter_mut().find(|c| c.id == *id) {
            cred.group_id = Some(group_id.to_string());
        }
        count += 1;
    }
    Ok(count)
...
/// 移动单个凭据到分组
pub fn move_credential_to_group(store: &mut StoreData, cred_id: &str, group_id: &str) -> Result<(), StorageError> {
    for group in &mut store.groups {
        group.entries.retain(|eid| eid != cred_id);
    }
    add_credential_to_group(store, cred_id, group_id)?;
    if let Some(cred) = store.credentials.iter_mut().find(|c| c.id == cred_id) {
        cred.group_id = Some(group_id.to_string());
    }
    Ok(())
}
...
/// 根据凭据环境自动分配系统分组
pub fn resolve_group_id(store: &StoreData, credential: &crate::models::Credential) -> Option<String> {
