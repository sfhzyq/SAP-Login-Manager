...
impl WindowsWindowInner {
    ...
    fn handle_hit_test_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
        ...
        // 补丁：左/右/底边缘缩放命中。原实现只放大了顶边（上方 y<=frame_y 分支），
        // 左右底仅剩 WM_NCCALCSIZE 保留的系统边框（~4px，实际几乎抓不到），
        // 导致自绘标题栏窗口拖宽后无法拖回。此处与顶边同宽补齐其余边缘，
        // 仅在标题栏元素未消费命中（drag_area 为 None）且未最大化时生效。
        if drag_area.is_none() && self.is_resizable && !self.state.is_maximized() {
            let mut client_rect = Default::default();
            unsafe { GetClientRect(handle, &mut client_rect) }.log_err();
            let in_left = cursor_point.x <= frame_x;
            let in_right = cursor_point.x >= client_rect.right - frame_x;
            let in_bottom = cursor_point.y >= client_rect.bottom - frame_y;
            if in_bottom {
                return Some(if in_left {
                    HTBOTTOMLEFT
                } else if in_right {
                    HTBOTTOMRIGHT
                } else {
