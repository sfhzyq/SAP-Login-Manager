...
use sap_backend::commands::{auth, sap_logon, transfer};
...
pub struct SapApp {
    ...
    /// 窗口置顶状态（自绘标题栏置顶按钮用）
    pinned: bool,
    /// 系统深浅色变化订阅（theme=system 时跟随系统切换，需保活防取消）
...
impl SapApp {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        // 系统深浅色变化监听：主题=跟随系统时自动切换（gpui 平台层收 WM_SETTINGCHANGE）
        let _appearance_sub = window.observe_window_appearance({
            let store = store.clone();
            move |window, cx| {
                let mode = store.lock().settings.theme.clone();
                if mode == "system" {
                    crate::state::apply_theme("system", Some(window), cx);
                }
            }
        });
    ...
    /// `danger`：关闭钮走 Win11 规范——悬停 #C42B1C 红底白图标（其余为浅灰）。
    fn title_control_btn(
        ...
        let (hover_bg, hover_fg): (gpui_kit::Hsla, gpui_kit::Hsla) = if danger {
            (rgba(0xC42B1CFF).into(), rgba(0xFFFFFFFF).into())
        } else {
        ...
        h_flex()
                ...
                div()
                    ...
                    .children((!locked).then(|| {
                        div()
                            ...
                            // 应用名加深：muted_foreground 太淡（用户反馈），用 tokens 次要文字色
            ...
            .child(
                h_flex()
                    ...
                    .when(!locked, |row| {
                        row.child(
                            h_flex()
                                ...
                                .gap(px(6.))
                                .mr(px(4.))
                                // 打开 SAP Logon（运行中 → 还原并置前台；未运行 → 启动）
                                .child(
                                    Button::new("tb-saplogon")
                                        .ghost()
                                        .small()
                                        .icon(IconName::AppWindow)
                                        .tooltip(t("打开 SAP Logon"))
                                        .on_click(cx.listener(|_, _, window, cx| {
                                            let path = {
                                                let s = AppState::global(cx).store.lock();
                                                s.settings.sap_logon_path.clone()
                                            };
                                            match sap_logon::open_or_focus(path.as_deref()) {
                                                Ok("opened") => window.push_notification(
                                                    t("SAP Logon 已启动"),
                                                    cx,
                                                ),
                                                Ok("focused") => window.push_notification(
                                                    t("SAP Logon 已置于前台"),
                                                    cx,
                                                ),
                                                Ok("running") => window.push_notification(
                                                    t("SAP Logon 正在运行"),
                                                    cx,
                                                ),
                                                Ok(_) => {}
                                                Err(e) => window.push_notification(
                                                    tf("打开 SAP Logon 失败：{e}", &[&e]),
                                                    cx,
                                                ),
                                            }
                                        })),
                    ...
                    .child(self.title_control_btn(cx, "tb-min", IconName::Minus, gpui::WindowControlArea::Min, false))
                    .child(self.title_control_btn(cx, "tb-max", IconName::Square, gpui::WindowControlArea::Max, false))
                    .child(self.title_control_btn(cx, "tb-close", IconName::X, gpui::WindowControlArea::Close, true)),
            )
            .into_any_element()
    }
    ...
    ///       ─ ⚙设置 · ℹ关于 → (弹性)
    /// 打开的面板对应按钮高亮（左竖条 + 白图标）；导出下方为深浅模式快捷切换
    ///
    /// 配色（2026-09-15 方案 C"暗壳亮芯"）：活动栏固定深蓝灰 #1E293B，
    /// 与浅/深主题的内容区都形成层次锚点；浅色主题下尤其修正"整窗发淡"。
    fn render_activity_bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        // 双主题自适应配色（浅色浅壳 / 深色深壳），色值集中在 tokens.rs
        let abc = tokens::activity_bar_colors(cx);
        v_flex()
            .flex_none()
            .w(px(48.))
            .h_full()
            ...
            // ☀/☾ 深浅模式快捷切换（用户要求：导出下方 + 分隔符）。kind="theme"
            // 永不命中 panel_kind，不会误激活面板高亮；点击翻转主题并持久化
            .child(Self::activity_divider(cx))
            .child(self.activity_btn(
                "act-theme",
                if cx.theme().is_dark() {
                ...
                cx.listener(|_, _, window, cx| {
                    ...
                    apply_theme_and_save(next, window, cx);
                }),
                cx,
            ))
    ...
    /// 活动栏图标按钮（配色双主题自适应，见 tokens::activity_bar_colors）：
    /// - 深壳：未激活图标浅灰蓝，悬停白 8% / 按压白 20%；激活白图标 + 白 12% 胶囊 + 亮蓝竖条
    /// - 浅壳：未激活图标走 muted_foreground，悬停黑 6% / 按压黑 10%；激活 primary 图标
    ///   + primary 12% 胶囊 + primary 竖条
                ...
                Button::new(id)
                    ...
                    .custom(
                        ButtonCustomVariant::new(cx)
                            .hover(abc.hover_pill)
                            .active(abc.press_pill),
                    )
                    .tooltip(tooltip)
                    .when(active, |t| t.bg(abc.active_pill).text_color(abc.active_fg))
                    .when(!active, |t| t.text_color(abc.idle_fg))
                    .on_click(on_click),
            )
    }

    /// 活动栏分区分隔线（24×1 水平线，深壳白 14% / 浅壳黑 14%）
    fn activity_divider(cx: &Context<Self>) -> impl IntoElement {
        div()
            ...
            .h(px(1.))
            .rounded_full()
            .bg(tokens::activity_bar_colors(cx).divider)
    }
}
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(page) = self.main_page.clone() {
            ...
            root.child(self.render_activity_bar(cx))
                ...
                // 弹窗层限制在活动栏右侧区域（absolute 容器即 Dialog 的 containing block）：
                // 设置等弹窗不再盖住左侧 48px 活动栏；锁定分支的弹窗保持全窗覆盖
                .children(dialog_layer.map(|layer| {
                    div()
                        .absolute()
                        .top_0()
                        .bottom_0()
            ...
            //
            // 整体绘制在高优先级 deferred 层（300 > gpui-kit tooltip 浮层的 200）：
            // tooltip 浮层挂在 Root 顶层且无公开的清除 API，悬停元素触发 tooltip
            // 后若被自动锁定卸载（如空闲超时锁定时鼠标正停在按钮上），浮层会
            // 残留在锁定页上方持续显示（上游修复 PR #2659 已被放弃，暂无库层
            // 修复）。这里用更高绘制优先级的锁定层盖住残留浮层；解锁回主界面
            // 后悬停机制自动恢复，无需额外处理。
            // 背景垫同色不透明底：Mica 模式下页面背景带 alpha，盖不住不透明的
            // tooltip 胶囊本体，必须先垫一块去掉 alpha 的同色纯色底。
            let mut opaque_bg = cx.theme().background;
            opaque_bg.a = 1.0;
            let locked = div()
                .absolute()
                .inset_0()
            ...
            gpui::deferred(locked).with_priority(300).into_any_element()
        }
    }
}
...
impl Render for AboutPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // 锚定面板容器 + 整页滚动（致谢列表较长，小窗口下需要滚动）+ 滚动条指示
        ...
        v_flex()
            ...
            .py(px(24.))
            .items_center()
            // Logo：与任务栏图标同构（蓝色圆角方底 + log-in 线性符号）
            .child(
                div()
                    .flex()
                    .items_center()
                    ...
                    .child(
                        svg()
                            .path(IconName::LogIn.path())
                            .size(px(28.))
                            .text_color(cx.theme().primary_foreground),
                    ),
            )
            // 名称 + 版本胶囊（版本号改为 primary 10% 底的小胶囊，替代纯文本行）
            .child(
                h_flex()
                    .items_center()
                    .gap(px(8.))
            ...
            .child(
                h_flex()
                    .items_center()
                    .gap(px(6.))
                    .child(Self::tech_chip("Rust", cx))
                    .child(Self::tech_chip(t("GPUI 原生"), cx))
                    .child(Self::tech_chip("Argon2id", cx)),
            )
            // 项目主页（GitHub 仓库）——不放明文 URL（用户反馈），
            // 链接通过「项目主页」按钮打开、复制按钮获取
            .child(
                h_flex()
                    .items_center()
                    .gap(px(6.))
...
impl AboutPanel {
    /// 技术徽章胶囊（关于页：Rust / GPUI 原生 / Argon2id）
    fn tech_chip(label: &str, cx: &Context<Self>) -> impl IntoElement {
        div()
            .px(px(10.))
            .py(px(3.))
            .rounded_full()
