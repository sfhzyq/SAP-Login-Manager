...
impl MainPage {
        ...
        self.filtered = self
            ...
            .filter(|c| {
                ...
                let ms = if q.is_empty() {
                    ...
                    if !grp_filters
                        .iter()
                        .all(|g| group_name.to_lowercase().contains(g))
                    {
                        return false;
                    }
            ...
            .child(
                Popover::new("group-popover")
                    ...
                    .content(move |_, _, cx| {
                        ...
                        let (sel, all_count, fav_count, sys_rows, custom_rows, collapsed_system, collapsed_custom) = {
                            let p = page.read(cx);
                            let row = |g: &Group| Row {
                                id: g.id.clone(),
    ...
    fn render_batch_bar(&self, cx: &mut Context<Self>) -> impl IntoElement + use<> {
        ...
        // 移动分组选项：默认 + 自定义分组（与 Tauri 版一致）
        let mut move_options: Vec<(String, String)> =
            vec![("".to_string(), t("默认分组").to_string())];
        for g in &self.groups {
            if !g.is_system && !g.is_default {
                move_options.push((g.id.clone(), g.group_name.clone()));
            }
        }
                ...
                h_flex()
                    ...
                    .child(
                        Button::new("batch-move")
                            ...
                            .dropdown_menu(move |menu, _, _| {
                                let weak = weak.clone();
                                let mut menu = menu.label(t("移动到分组"));
                                for (gid, name) in move_options.iter().cloned() {
                                    let weak = weak.clone();
                                    menu = menu.item(
                                        PopupMenuItem::new(name).on_click(move |_, window, cx| {
                                            if let Some(page) = weak.upgrade() {
                                                page.update(cx, |page, cx| {
                                                    page.batch_move(gid.clone(), window, cx);
                                                });
                                            }
                                        }),
                                    );
