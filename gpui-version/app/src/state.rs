...
fn customize_theme_defaults(cx: &mut App) {
    ...
    // 三层背景（2026-09-15 配色加深，修正"整体偏淡"）：
    // 主界面 #E2E6EF（90% alpha 仍透 Mica 壁纸色调）/ 弹窗+卡片 白 / 子容器+chip #D8DDE8；
    // sidebar 仅剩导入表格盒使用（与卡片统一白底）；活动栏深壳配色在 ui/mod.rs 硬编码
    light.colors.background = Some("#E2E6EFE6".into());
    light.colors.popover = Some("#ffffff".into());
    light.colors.secondary = Some("#D8DDE8".into());
    light.colors.sidebar = Some("#FFFFFF".into());
    light.colors.title_bar = Some("#E2E6EFE6".into());
    // 文本：主文本深 / 弱化文本中灰
    light.colors.foreground = Some("#1a1d2e".into());
    light.colors.muted_foreground = Some("#7A8299".into());
    // 边框与危险色（随底色加深同步加深一档，保证卡片边界可辨）
    light.colors.border = Some("#CFD5E2".into());
    ...
    // （同浅色：底色 70% alpha 透 Mica）
    dark.colors.background = Some("#0A0B14B3".into());
    dark.colors.popover = Some("#14161f".into());
    dark.colors.secondary = Some("#1e2130".into());
    dark.colors.sidebar = Some("#0A0B14B3".into());
    dark.colors.title_bar = Some("#0A0B14B3".into());
    // 文本：主文本近白 / 弱化文本中灰
    ...
    // 通知弹窗（toast）移至右下角：kit 默认 TopRight 出现在窗口顶部，
    // 382px 宽的 toast 会盖住面板标题（如"分组管理"，用户反馈截图）。
    // Theme::change 切主题时的 apply_config 不重置 notification 字段，
    // 这里设置一次即可长期生效；margins 沿用默认（距边 16px）。
    theme.notification.placement = gpui_kit::gpui::Anchor::BottomRight;
}
